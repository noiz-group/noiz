...
def test_catching_integrity_check_warning():
    ...
    warning_message = ("InternalMSEEDWarning: EN_ES23_00_CHE_D: Warning: Data integrity check for Steim1 failed, "
    ...
    with pytest.raises(Warning):
        ...
        with CatchWarningAsError(
            ...
            warnings.warn(warning_message, InternalMSEEDWarning)
...
def test_catching_custom_warning():
    ...
    with pytest.raises(Warning):
        with CatchWarningAsError(
                warning_filter_action="error",
                warning_filter_message="(?s).*warning(?s).*"
        ):
