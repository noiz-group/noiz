...
def run_chunk_processing(
    ...
    with app.app_context():
        ...
        for i, dc in enumerate(datachunks):
            ...
            logging.info(f"Loading data for {dc}")
            st = dc.load_data()
            if len(st) != 1:
                logging.info(
            ...
            new_filepath = dc.filepath.replace("datachunk", "processed_datachunk")
            ...
            existing_chunks = (
                ...
                .filter(
                    ...
                    ProcessedDatachunk.processing_params_id
                    == proc_dc.processing_params_id,
                )
                .all()
            )
            logging.info(
